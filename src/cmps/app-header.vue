<template>
  <header :style="{ 'background-color': getCalcColor?.calcColor }" class="app-header">
    <nav>
      <section class="trello">
        <router-link to="/board">
          <button class="logo">
            <font-awesome-icon :style="!getCalcColor?.isDark ? { color: 'black' } : ''" class="icon"
              icon="fa-brands fa-trello" />
            <span class="title" :style="!getCalcColor?.isDark ? { color: 'black' } : ''">Portobello</span>
          </button>
        </router-link>
        <section class="trello-actions">
          <Popper offsetSkid="110" class="popper-main">
            <button class="nav-item">
              ...
              <svg :style="!getCalcColor?.isDark ? { color: 'black' } : ''" width="18" height="18" role="presentation"
  ...
  (board) => board.isStarred
                  ...
                  <label v-else>Star important boards to access them quickly and
                    easily.</label>
                </ul>
              </div>
            </template>
        ...
        <section class="trello-actions-more">
          <button class="nav-item">
            ...
            <svg :style="!getCalcColor?.isDark ? { color: 'black' } : ''" width="18" height="18" role="presentation"
              focusable="false" viewBox="0 0 30 25" xmlns="http://www.w3.org/2000/svg">
              <path
                d="M11.2929 16.7071L4.22185 9.63606C3.83132 9.24554 3.83132 8.61237 4.22185 8.22185C4.61237 7.83133 5.24554 7.83133 5.63606 8.22185L12 14.5858L18.364 8.22185C18.7545 7.83132 19.3877 7.83132 19.7782 8.22185C20.1687 8.61237 20.1687 9.24554 19.7782 9.63606L12.7071 16.7071C12.3166 17.0977 11.6834 17.0977 11.2929 16.7071Z"
                fill="currentColor"></path>
            </svg>
      ...
      <button class="humburger">
        <font-awesome-icon :style="!getCalcColor?.isDark ? { color: 'black' } : ''" icon="fa-solid fa-bars"
          class="icon" />
      </button>
      <section class="user">
        <div class="search">
          ...
          <span :style="!getCalcColor?.isDark ? { color: 'black' } : ''" class="icon">
            <svg :style="!getCalcColor?.isDark ? { color: 'black' } : ''" class="icon-svg" width="24" height="24"
